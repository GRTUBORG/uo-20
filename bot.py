...
def buildings(message):
    ...
    url_button = types.InlineKeyboardButton(text = "Открыть интерактивную карту ↗️", url = 'https://bit.ly/3sRomy6')
...
def callback_inline(call):
    if call.message:
        if call.data == 'adress_1':
            ...
            keyboard = types.InlineKeyboardMarkup()
            button = types.InlineKeyboardButton(text = "В чат к разработчику ↗️", url = 'https://t.me/whomet')
            button1 = types.InlineKeyboardButton(text = "⬅️ Назад в меню", callback_data = 'back_to_the_menu')
            keyboard.row(button)
            keyboard.row(button1)
